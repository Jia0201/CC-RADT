...
Skills are packaged expertise with workflows, reference materials, and best practices. Create them in `skills/agents/<agent>/<skill>/SKILL.md`. Skills can be invoked by Claude automatically when relevant, or by users directly with `/skill-name`.
...
Create project-specific skills in `skills/agents/<agent>/<skill>/SKILL.md`.
...
```
skills/agents/<agent>/create-migration/
├── SKILL.md
└── scripts/
...
3. Run validation: `bash skills/agents/<agent>/create-migration/scripts/validate-migration.sh`
...
```
skills/agents/<agent>/new-component/
├── SKILL.md
└── templates/
