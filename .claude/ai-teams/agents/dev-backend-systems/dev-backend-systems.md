---
id: "agents-dev-backend-systems-dev-backend-systems"
title: "Dev-Backend-Systems 系统后端 Agent"
type: "agent-profile"
scope: "agent"
owner: "role"
status: active
...
---
# Dev-Backend-Systems 系统后端 Agent
...
- [专属 Skills](../../skills/agents/dev-backend-systems/index.md)
...
- 开发类任务必须读取并遵守 [development-policy](../../security/development-policy.md) 和 [development-backend-systems-policy](../../security/development-backend-systems-policy.md) 中的开发规则、禁区、语法要求和验证要求。
...
1. 读取 [index](../../rule/index.md)、[dev-backend-systems](../../rule/agents/dev-backend-systems.md) 和 [index](../../rule/tasks/index.md)，先获得当前 Agent 与任务的最小读取集合。
...
4. 按路由读取 [playbook](../../playbook.md)、[dev-backend-systems](../../security/agent-playbooks/dev-backend-systems.md) 和必要的安全规则，不默认加载整个 `security/`。
...
- 项目类任务先读取 [dev-backend-systems](../../rule/agents/dev-backend-systems.md) 和 [index](../../rule/project/index.md)，再按任务路由读取必要项目事实，不得默认加载整个 `project/`。
...
- 正式 Agent 记忆：[MEMORY](../../memory/agents/dev-backend-systems/MEMORY.md)
- 共享正式记忆：[MEMORY](../../memory/MEMORY.md)

## 关联知识库

- 本地知识库指针：[kb](./kb.md)
- 正式 Agent 知识入口：[index](../../kb/agents/dev-backend-systems/index.md)
- 知识图谱入口：[graph](../../kb/graph.md)

## 关联 Skills

- 本地 Skills 指针：[skills](./skills.md)
- Agent Skills 目录：`skills/agents/dev-backend-systems/`
- Skills 注册表：`skills/registry.json`

## 关联 MCP

- 本地 MCP 指针：[mcp](./mcp.md)
- Agent MCP 目录：`mcp/agents/dev-backend-systems/`
- MCP 绑定索引：[index](../../mcp/agents/dev-backend-systems/index.md)
- MCP 注册表：`mcp/registry.json`

## 关联提示词

- 专属 Prompt：[index](../../prompts/agents/dev-backend-systems/index.md)
- 活动版本：`prompts/registry.json`
- 失败事件：[index](../../shared/prompt-evolution/index.md)
- 接口、构建或项目事实错误优先更新契约与 `project/`；只有提示词根因才提交候选建议，不修改 active。
