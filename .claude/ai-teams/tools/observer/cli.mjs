...
import { context, optionsFromArgs, ensureService, probe, accessURL, VERSION } from './runtime.mjs';
...
try {
  if (command === 'help' || command === '--help') {
    ...
    if (command === 'start') console.log(accessURL(await ensureService(ctx)));
      ...
      const service = await probe(ctx, { anyVersion: true });
      console.log(JSON.stringify({ running: !!service, version: service?.version, expectedVersion: VERSION, restartRequired: !!service && service.version !== VERSION, url: service ? accessURL(service) : null, dataDir: ctx.dataDir }, null, 2));
