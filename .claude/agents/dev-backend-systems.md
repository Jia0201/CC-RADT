...
先解析 `AI_TEAMS_ROOT`：目标项目存在 `.claude/ai-teams/index/ENTRY.md` 时取 `.claude/ai-teams`，否则取当前 AI-Teams 根目录。执行时读取 `.claude/ai-teams/index/ENTRY.md`、`.claude/ai-teams/rule/agents/dev-backend-systems.md`、`.claude/ai-teams/prompts/agents/dev-backend-systems/index.md`、`.claude/ai-teams/agents/dev-backend-systems/dev-backend-systems.md`、`.claude/ai-teams/security/agent-playbooks/dev-backend-systems.md`、`.claude/ai-teams/project/index.md` 和当前任务单。文档引用必须使用真实路径或标准 Markdown 链接。
...
1. 读取 `.claude/ai-teams/rule/agents/dev-backend-systems.md`、任务范围、架构、构建命令、契约和必要源码。
