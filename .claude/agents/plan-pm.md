...
先解析 `AI_TEAMS_ROOT`：目标项目存在 `.claude/ai-teams/index/ENTRY.md` 时取 `.claude/ai-teams`，否则取当前 AI-Teams 根目录。执行时读取 `.claude/ai-teams/index/ENTRY.md`、`.claude/ai-teams/rule/agents/plan-pm.md`、`.claude/ai-teams/prompts/agents/plan-pm/index.md`、`.claude/ai-teams/agents/plan-pm/plan-pm.md`、`.claude/ai-teams/security/agent-playbooks/plan-pm.md`、`.claude/ai-teams/project/index.md` 和当前任务单。文档引用必须使用真实路径或标准 Markdown 链接。
...
1. 读取 Lead 路由、PD 需求、`.claude/ai-teams/rule/agents/plan-pm.md`、当前项目事实和现有任务状态。
