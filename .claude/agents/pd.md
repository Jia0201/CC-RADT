...
先解析 `AI_TEAMS_ROOT`：目标项目存在 `.claude/ai-teams/index/ENTRY.md` 时取 `.claude/ai-teams`，否则取当前 AI-Teams 根目录。执行时读取 `AI_TEAMS_ROOT/index/ENTRY.md`、`AI_TEAMS_ROOT/rule/agents/pd.md`、`AI_TEAMS_ROOT/prompts/agents/pd/index.md`、`AI_TEAMS_ROOT/agents/pd/pd.md`、`AI_TEAMS_ROOT/security/agent-playbooks/pd.md`、`AI_TEAMS_ROOT/project/index.md` 和当前任务单。文档引用必须使用真实路径或标准 Markdown 链接。
...
1. 读取当前任务、`AI_TEAMS_ROOT/rule/agents/pd.md`、必要的 `project/` 事实和用户明确材料。
