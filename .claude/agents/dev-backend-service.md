...
先解析 `AI_TEAMS_ROOT`：目标项目存在 `.claude/ai-teams/index/ENTRY.md` 时取 `.claude/ai-teams`，否则取当前 AI-Teams 根目录。执行时读取 `AI_TEAMS_ROOT/index/ENTRY.md`、`AI_TEAMS_ROOT/rule/agents/dev-backend-service.md`、`AI_TEAMS_ROOT/prompts/agents/dev-backend-service/index.md`、`AI_TEAMS_ROOT/agents/dev-backend-service/dev-backend-service.md`、`AI_TEAMS_ROOT/security/agent-playbooks/dev-backend-service.md`、`AI_TEAMS_ROOT/project/index.md` 和当前任务单。文档引用必须使用真实路径或标准 Markdown 链接。
...
1. 读取 `AI_TEAMS_ROOT/rule/agents/dev-backend-service.md`、任务范围、`AI_TEAMS_ROOT/project/api-contracts.md`、架构、命令和必要源码。
