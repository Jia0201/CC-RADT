...
- [专属 Skills](../../skills/agents/pd/index.md)
...
1. 读取 [index](../../rule/index.md)、[pd](../../rule/agents/pd.md) 和 [index](../../rule/tasks/index.md)，先获得当前 Agent 与任务的最小读取集合。
...
4. 按路由读取 [playbook](../../playbook.md)、[pd](../../security/agent-playbooks/pd.md) 和必要的安全规则，不默认加载整个 `security/`。
...
- 需求解析只依据用户当前需求、目标项目当前文件、已确认项目文档和 `project/` 当前事实；Git 分支、HEAD 和工作区变更数量只能作为辅助环境信号。
- 不读取、不总结、不引用 Git 提交历史、提交正文、历史代码或大范围 diff 作为需求依据，除非用户明确要求“分析 Git 历史”。
- 默认输出轻量“需求卡”，只写用户目标、业务字段、验收口径、待确认问题、不做范围和交给 Plan-PM 的最小可计划任务。
- 只有跨模块、跨端、合规、安全、架构或用户明确要求完整需求文档时，才升级为完整需求文档。
- 页面和接口相关需求必须把字段、展示名、必填、默认值、枚举、空态、错误态和权限态列清楚；不确定就标为待确认。

## 目标项目管理规则

- 项目类任务先读取 [pd](../../rule/agents/pd.md) 和 [index](../../rule/project/index.md)，再按任务路由读取必要项目事实，不得默认加载整个 `project/`。
...
- 当前 Agent 的项目职责、正式写入边界和交接要求以 [pd](../../rule/agents/pd.md)、本 Agent role 和专属 playbook 为准。
...
- 不绕过 Lead 直接分派 Dev。
- 不把未确认猜测写入正式知识库或正式记忆。
- 不把 Git 提交历史、提交正文、历史代码或 diff 当作需求来源；Git 命令不可用时直接跳过并回到项目文件与用户确认信息。

## 关联记忆

- 本地记忆指针：[memory](./memory.md)
- 正式 Agent 记忆：[MEMORY](../../memory/agents/pd/MEMORY.md)
- 共享正式记忆：[MEMORY](../../memory/MEMORY.md)

## 关联知识库

- 本地知识库指针：[kb](./kb.md)
- 正式 Agent 知识入口：[index](../../kb/agents/pd/index.md)
- 知识图谱入口：[graph](../../kb/graph.md)

## 关联 Skills

- 本地 Skills 指针：[skills](./skills.md)
- Agent Skills 目录：`skills/agents/pd/`
- Skills 注册表：`skills/registry.json`

## 关联 MCP

- 本地 MCP 指针：[mcp](./mcp.md)
- Agent MCP 目录：`mcp/agents/pd/`
- MCP 绑定索引：[index](../../mcp/agents/pd/index.md)
- MCP 注册表：`mcp/registry.json`

## 关联提示词

- 专属 Prompt：[index](../../prompts/agents/pd/index.md)
- 活动版本：`prompts/registry.json`
- 失败事件：[index](../../shared/prompt-evolution/index.md)
- 发现稳定需求解析缺陷时提交脱敏事实和建议，不修改自己的活动 system prompt。
