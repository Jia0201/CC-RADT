---
id: "agents-dev-frontend-miniapp-workflow"
title: "Dev-Frontend-Miniapp 工作流"
type: "agent-workflow"
scope: "agent"
owner: "role"
status: active
---
# Dev-Frontend-Miniapp 工作流

本文件是 Dev-Frontend-Miniapp 的工作流结构文件。Agent 主入口保留为 `agents/dev-frontend-miniapp/dev-frontend-miniapp.md`。
...
Dev-Frontend-Miniapp 按 [playbook](../../playbook.md) 接收小程序端任务，优先对齐平台差异、授权、支付、分享、分包、[api-contracts](../../project/api-contracts.md) 和 [index](../../shared/contracts/index.md)。

| 参与方式 | WF 编号 | 参与重点 |
|---|---|---|
| 主责 | `WF-04` | 处理微信/支付宝/uni-app 等小程序页面、组件、授权、支付、分享、分包和平台状态，完成后提交平台验证结果和未验证项 |
| 协作 | `WF-02` / `WF-07` / `WF-08` / `WF-10` / `WF-11` | `WF-02` 处理低风险小程序小修；`WF-07` 与后端按字段契约联调；`WF-08` 接 QA 复现结论修复小程序缺陷；`WF-10` 参与标准功能开发；`WF-11` 只在 Security-Reviewer 允许范围内改动 |
| 旁路监督 | `WF-05` / `WF-06` / `WF-12` | 后端服务或系统后端变更影响小程序授权、支付、接口字段、错误码或平台状态时提出影响；文档/图谱流中向 Doc 提供平台差异事实 |
