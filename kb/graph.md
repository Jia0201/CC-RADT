...
工作流选择器的规则本体在 [playbook.md 3.4](../playbook.md#3.4-工作流选择器)；Agent 执行入口仍是 `agents/<agent>/workflow.md` 和 `security/agent-playbooks/<agent>.md`。本节只维护图谱关系，不复制完整动作规则。长期设计原因见 [ADR-0010 工作流选择器](../project/adr/accepted/ADR-0010-workflow-selector.md)。
...
graph TD
  ...
  WorkflowSelector --> ADR0010["project/adr/accepted/ADR-0010-workflow-selector.md"]
...
| No. | 指令 | 入口 |
|---:|---|---|
| 1 | 一键打包指令（正式版） | [package-formal](../tools/commands/ai/package-formal.md) |
| 2 | 一键打包指令（精简版） | [package-simplify](../tools/commands/ai/package-simplify.md) |
...
- formal 正式版打包链路已进入当前图谱主线；精简版、OpenCode 版本和 Codex 版本后续继续收敛。
