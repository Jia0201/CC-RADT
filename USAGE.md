...
观察台不执行命令、不审批、不写配置。V2 可从当前项目 CC 本地会话中读取脱敏公开对话、工具输入输出、修改片段与已报告 Token，不采集隐藏思考，不复制原始会话文件。缺失记录显示未提供；磁盘角色定义不代表实际加载状态，当前 Git diff 与会话修改片段分开展示，不能自动归因。目录新增 Skills、MCP 与软链路，完整边界和关闭原生对话读取的开关见 [观察台说明](.claude/ai-teams/tools/observer/README.md)。跨项目聚合、SSE 和费用统计尚未实现。
